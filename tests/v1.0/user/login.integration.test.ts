...
import { validateLoginUser } from '../../../src/middleware/validators/user/validateLoginUser';
...
app.post(path, validateLoginUser, login);
	...
	describe('Failure cases', () => {
		...
		describe('Invalid fields', () => {
			it('should fail when email is invalid', async () => {
				const response = await supertest(app)
					.post(path)
					.send({ email: 'invalidEmail', password: mockUser.password });
				expect(response.status).toBe(400);
				expectExpressValidatorError(response, 'email');
				expect(userFindOneSpy).not.toHaveBeenCalled();
			});
			it('should fail when password is too short', async () => {
				const response = await supertest(app)
					.post(path)
					.send({ email: mockUser.email, password: 'short' });
				expect(response.status).toBe(400);
				expectExpressValidatorError(response, 'password');
				expect(userFindOneSpy).not.toHaveBeenCalled();
			});
		...
		it('should not login a user with invalid credentials', async () => {
			...
			const response = await supertest(app)
				.post(path)
				.send({ email: unauthorizedEmail, password: 'password123' });
