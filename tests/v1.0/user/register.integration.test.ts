...
import { validateRegisterUser } from '../../../src/middleware/validators/user/validateRegisterUser';
import register from '../../../src/api/v1.0/user/register';
import User from '../../../src/models/User';

const app = express();
app.use(express.json());
const path = '/v1.0/user/register';
app.post(path, validateRegisterUser, register);
	...
	describe('Failure Cases', () => {
		...
		describe('Missing fields', () => {
			it('should fail when no payload is provided', async () => {
				...
				expect(response.status).toBe(400);
				expectExpressValidatorError(response, 'username');
			...
			it('should fail when email is not provided', async () => {
				const response = await supertest(app)
					.post(path)
					.send({ password: mockUser.password, username: mockUser.username });
				expect(response.status).toBe(400);
				expectExpressValidatorError(response, 'email');
				expect(userCreateSpy).not.toHaveBeenCalled();
			});
			it('should fail when password is not provided', async () => {
				const response = await supertest(app)
					.post(path)
					.send({ email: mockUser.email, username: mockUser.username });
				expect(response.status).toBe(400);
				expectExpressValidatorError(response, 'password');
				expect(userCreateSpy).not.toHaveBeenCalled();
			});
